...
- `docs/` — Astro + Paraglide JS static site (GitHub Pages landing page at `https://fidoscan.thelukez.com/`)
  - `docs/src/pages/index.astro` — root redirect; detects browser language and redirects to `/{locale}/`
  - `docs/src/pages/[locale]/index.astro` — main page; SSG with per-locale static output
  - `docs/messages/{locale}.json` — translation source files (en, de); add keys here to add new strings
  - `docs/src/paraglide/` — generated by `paraglide-js compile` (do not edit directly)
  - `docs/project.inlang/settings.json` — inlang project config (locales, message format plugin)

## Distribution

- **GitHub Releases** — signed APK attached as `fido-scanner-vX.Y.Z.apk` on each release
- **GitHub Pages** — Astro static site in `docs/`, built output served from `main:/docs`; fetches latest release via GitHub API at runtime
